...
import type { StaffContractDto, StaffDto, TrainerDto } from "@thoroughline/contracts";
import {
  generateTrainer,
  maxTrainers,
  Rng,
...
export class StaffService {
  ...
  async mine(userId: string): Promise<StaffDto> {
    ...
    return {
      contracts,
      maxTrainers: maxTrainers(stable.level, this.config.get()),
      weeklyCost: contracts.reduce((s, c) => s + c.salary, 0),
    };
  }
  ...
  async hire(userId: string, trainerId: string): Promise<StaffDto> {
    ...
    await this.db.tx(async (c) => {
      ...
      const active = await row<{ n: number }>(
        c,
        "SELECT count(*)::int AS n FROM staff_contracts WHERE owner_id = $1 AND ended_at IS NULL",
        [userId],
      );
  ...
  async dismiss(userId: string, contractId: string): Promise<StaffDto> {
    ...
    await this.db.tx(async (c) => {
      ...
      await this.events.emit(c, {
        type: "trainer_dismissed",
        aggregateType: "trainer",
        aggregateId: k.trainer_id,
        actorId: userId,
        payload: { contractId },
      });
    });
  ...
  async renew(contractId: string): Promise<boolean> {
    ...
    return this.db.tx(async (c) => {
      const k = await row<ContractRow & { name: string }>(
        c,
        `SELECT k.*, t.name FROM staff_contracts k JOIN trainers t ON t.id = k.trainer_id
